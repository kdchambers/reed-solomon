...
pub inline fn divide(a: u8, b: u8) u8 {
    ...
    const log_result = blk: {
        const tmp = @intCast(i64, log_a) - @intCast(i64, log_b);
        break :blk if (tmp < 0) tmp + 255 else tmp;
    };
    return exponent_table[@intCast(usize, log_result)];
}

pub inline fn exp(a: u8, n: u8) u8 {
    if (n == 0)
        return 1;
    if (a == 0)
        return 0;
    const log_a = log_table[a];
    const log_result = (log_a * n) % 255;
    return exponent_table[log_result];
}

pub inline fn mult(a: u8, b: u8) u8 {
    if (a == 0 or b == 0)
        return 0;
    const log_a = log_table[a];
    const log_b = log_table[b];
    return exponent_table[log_a + log_b];
}

const null_log_entry = std.math.maxInt(usize);

const log_table = [_]usize{
    null_log_entry, 0,   1,   25,  2,   50,  26,  198,
